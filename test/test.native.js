...
var dmaxabs = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( dmaxabs.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum absolute value of a one-dimensional ndarray', opts, function test( t ) {
	var x;
	var v;

	x = new Float64Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = dmaxabs( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 5.0, 'returns expected value' );

	x = new Float64Array( [ -4.0, -5.0 ] );
	v = dmaxabs( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 5.0, 'returns expected value' );

	x = new Float64Array( [ -0.0, 0.0, -0.0 ] );
	v = dmaxabs( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( v, 0.0, 'returns expected value' );

	x = new Float64Array( [ NaN ] );
	v = dmaxabs( [ vector( x, 1, 1, 0 ) ] );
